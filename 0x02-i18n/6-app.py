...
def get_locale():
    ...
    """
    # Check if the current user is set and has a preferred locale
    x = get_user()
    if (hasattr(g, 'x') and g.x and 'locale' in g.x):

        user_locale = g.x['locale']
        # If the user's preferred locale is supported, return it
    ...
    # If the user's preferred locale is not supported or not set,
